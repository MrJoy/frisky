require 'rack'
require_relative '../frisky/control_point'

module Rack

  # Middleware that allows your Rack app to keep tabs on devices that the
  # {Frisky::ControlPoint} has found.  UPnP devices that match the +search_type+
  ...
  # have one running, the {Frisky::ControlPoint} will start one for you.
  #
  ...
  class UPnPControlPoint
    ...
    # @param [Hash] options Options to pass to the Frisky::SSDP::Searcher.
    # @see Frisky::SSDP::Searcher
    ...
    # Creates and starts the {Frisky::ControlPoint}, then manages the list of
    ...
    def start_control_point(search_type, options)
      @cp = ::Frisky::ControlPoint.new(search_type, options)
